...
class HealpixTest(MPITestCase):
    ...
    def test_ang_vec(self):
        ...
        for nside in [1, 256, 16384]:
            (regular, regcompnest, regcompring, extremes, extcompnest, extcompring) = self.create_hp_data(nside)
    ...
    def test_degrade_upgrade(self):
        ...
        for nside in [1, 256, 16384]:
            npix = 12 * nside ** 2
            ring_pix = np.linspace(0, npix-1, num=nsamp, endpoint=True, dtype=np.int64)
